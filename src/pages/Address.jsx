...
function AddressInfo({ balance, address, count }) {
  return (
    <div className="addressInfo">
      <div className="address-card">
        <div className="address-card-header">Address {address}</div>
        <div className="address-card-body">
          <div title={balance}>Balance: {Number(balance).toFixed(18)} ETH</div>
...
const Address = () => {
  ...
  useEffect(() => {
    async function getHistory() {
      ...
      console.log("historyFrom", historyfrom);
  ...
  return (
    <div className="page-content">
      ...
      <div className="transaction-history">
        ...
        <div>
          <button className="history-button" onClick={onTabClickE}>
            External
          </button>
          <button className="history-button" onClick={onTabClickI}>
            Internal
          </button>
          <button className="history-button" onClick={onTabClick20}>
            Erc20
          </button>
          <button className="history-button" onClick={onTabClick721}>
            Erc721
          </button>
          <button className="history-button" onClick={onTabClick1155}>
            Erc1155
          </button>
        </div>
        ...
        <table>
          ...
          <tbody>
            {showHistory?.map((item) => {
              return (
                <tr key={`${item.uniqueId}-1`}>
                  <td key={`${item.uniqueId}-2`} title={item.hash}>
                    <Link className="list-links" to={`/tx/${item.hash}`}>
                  ...
                  <td key={`${item.uniqueId}-4`} title={item.from}>
                    <Link className="list-links" to={`/address/${item.from}`}>
                      {shortenAddress(item.from)}
                    </Link>
                  </td>
                  <td key={`${item.uniqueId}-5`} title={item.to}>
                    <Link className="list-links" to={`/address/${item.to}`}>
